...
public class RequestController {
    ...
    public ResultVO getFriendRequests(@RequestAttribute TokenUtil.JwtClaimsData claims) {
        List<Request> requests = requestService.findByTargetId(claims.getId());
    ...
    public ResultVO getMyRequests(@RequestAttribute TokenUtil.JwtClaimsData claims) {
        List<Request> requests = requestService.findByOriginatorId(claims.getId());
        log.info("{} [Get /#/friends]: {}", claims.getId(), requests);
        return ResultVO.success(requests);
    }
    ...
    public ResultVO postAddFriendRequest(
        ...
        final String oppositeId = ((String) requestForm.getOrDefault("oppositeId", "")).trim();
        final String reason = ((String) requestForm.getOrDefault("reason", "")).trim();
    ...
    public ResultVO postConfirmFriend(
        ...
        final String type = ((String) (body.getOrDefault("type", "accept"))).trim();
        final String reason = ((String) (body.getOrDefault("reason", ""))).trim();
